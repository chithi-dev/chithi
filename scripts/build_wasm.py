...
export async function init() {
    ...
    ready = (async () => {
        ...
        if (typeof window === "undefined") {
            ...
            wasm = await WebAssembly.instantiateStreaming(fetch(path));
        }
        // Export functions globally on the module for direct access
        const exports = wasm.instance.exports;
        for (const name of Object.keys(wasm.instance.exports)) {
            (globalThis as any)[`__chithi_${name}`] = exports[name];
        }
    })();
    return ready;
}

// Re-export the memory for direct access
export function getMemory(): WebAssembly.Memory | null {
    return wasm ? wasm.instance.exports.memory as WebAssembly.Memory : null;
}

// Re-export the instance for advanced usage
export function getInstance(): WebAssembly.Instance | null {
    return wasm ? wasm.instance : null;
}

// Convenience: export the init as default
export default init;
'''
