...
fn py_upload(files: Vec<(String, Vec<u8>)>, password: String) -> PyResult<Vec<u8>> {
    Python::with_gil(|py| {
        py.allow_threads(|| {
            ...
            sdk_compress_and_encrypt(&files, Some(&password))
...
fn py_download(bundle: &[u8], password: String) -> PyResult<Vec<(String, Vec<u8>)>> {
    Python::with_gil(|py| {
        py.allow_threads(|| {
            ...
            sdk_decrypt_and_decompress(bundle, Some(&password))
                .map_err(|e| pyo3::exceptions::PyValueError::new_err(e))
        })
    })
}
...
fn pi_encrypt_chunks_parallel(
    ...
    Python::with_gil(|py| {
        py.allow_threads(|| {
            encrypt_chunks_parallel(&chunks, &key_arr, &base_iv_arr)
                .map_err(|e| pyo3::exceptions::PyValueError::new_err(e))
        })
    })
}
...
fn pi_decrypt_chunks_parallel(
    ...
    Python::with_gil(|py| {
        py.allow_threads(|| {
            decrypt_chunks_parallel(&chunks, &key_arr, &base_iv_arr)
                .map_err(|e| pyo3::exceptions::PyValueError::new_err(e))
        })
    })
}
