...
export default function HomeClient({ release }: { release: Release }) {
	...
	function detectOS(): "windows" | "macos" | "linux" | "unknown" {
		...
		if (
			...
			return "macos";
		if (tokens.has("linux") || tokens.has("ubuntu") || tokens.has("android"))
			return "linux";
		return "unknown";
	}
	...
	function findAssetForOS(rel: any, os: string) {
		...
		const normalizeKeyword = (k: string) => k.toLowerCase().replace(/^[.]/, "");
		...
		<main className="relative z-10 mx-auto w-full max-w-7xl px-6">
			...
			<section className="relative flex flex-col items-center justify-center pt-24 pb-20 text-center md:pt-40 md:pb-32">
				...
				<div className="max-w-4xl" data-aos="fade-up">
					...
					<p className="mx-auto mb-10 max-w-2xl font-light text-surface-600-400 text-xl leading-relaxed">
						Open-source, self-hostable, and encrypted by default. Built with
						RustFS for performance and FastAPI for reliability. Run it anywhere,
						inspect the code, and contribute.
					</p>
			...
			<section className="py-12 md:py-16">
				<div className="mx-auto max-w-3xl text-center" data-aos="fade-up">
					...
					<p className="mb-6 font-light text-lg text-surface-600-400 leading-relaxed">
						Official CLI releases are published on GitHub. Select your OS and
						architecture (or let us auto-detect), or build from source for
					...
					<div className="flex items-center justify-center gap-3">
						<select
							value={downloadOS}
							onChange={(e) =>
								setDownloadOS(e.target.value as typeof downloadOS)
							}
							className="select variant-form-material"
						>
							<option value="auto">Auto-detect{detectedOSLabel}</option>
						...
						<select
							value={downloadArch}
							onChange={(e) =>
								setDownloadArch(e.target.value as typeof downloadArch)
							}
							className="select variant-form-material"
						>
			...
			<section className="py-24 md:py-32">
				<div className="mx-auto mb-16 max-w-3xl text-center" data-aos="fade-up">
					...
					<p className="font-light text-lg text-surface-600-400 leading-relaxed">
						...
						Community-maintained examples and configs make it easy to run on a
						home server, VPS, or cloud.
					</p>
				</div>
			...
			<section className="border-surface-200-800/50 border-t py-24 md:py-32">
				<div className="flex flex-col items-center gap-16 lg:flex-row">
					<div className="flex-1 space-y-6" data-aos="fade-right">
						<h2 className="h2 font-bold tracking-tight">Built-in Speedtest</h2>
						<p className="font-light text-lg text-surface-600-400 leading-relaxed">
							Diagnose your network capabilities directly from the interface.
							Measure throughput to the server to ensure optimal file transfer
							rates before you start, all within the Chithi dashboard.
						</p>
					...
					<div className="w-full flex-1" data-aos="fade-left">
						<div className="card relative space-y-8 overflow-hidden rounded-3xl border border-surface-200-800 bg-surface-100-900/50 p-8">
							<div className="flex items-start justify-between">
								<div>
									<div className="mb-2 flex items-center gap-3">
										...
										<h3 className="h3 font-bold text-lg">Network Status</h3>
									</div>
								</div>
							...
							<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
								<div className="flex flex-col items-start justify-center rounded-2xl border border-surface-200-800 bg-surface-50-950/80 p-6">
									<div className="mb-4 flex items-center gap-2 font-medium text-surface-600-400 text-xs uppercase tracking-widest">
										<ArrowDown size={14} className="text-secondary-500" />{" "}
										Download
									</div>
								...
								<div className="flex flex-col items-start justify-center rounded-2xl border border-surface-200-800 bg-surface-50-950/80 p-6">
									<div className="mb-4 flex items-center gap-2 font-medium text-surface-600-400 text-xs uppercase tracking-widest">
										<ArrowUp size={14} className="text-tertiary-500" /> Upload
									</div>
									<div className="mb-1 font-bold text-3xl tracking-tight">
										396.6{" "}
										<span className="font-light text-sm text-surface-600-400">
							...
							<div className="space-y-3 pt-2">
								...
								<Progress value={21} max={100} className="h-1.5">
			...
			<section className="border-surface-200-800/50 border-t py-24 md:py-32">
				<div className="mb-16" data-aos="fade-up">
					...
					<p className="max-w-2xl font-light text-lg text-surface-600-400 leading-relaxed">
						Chithi comes with modular components supported out of the box,
						ensuring seamless integration whether you need a rich web UI, an
						embedded TUI, or a high-performance backend serving API traffic.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
					{ecosystem.map((item, i) => (
						...
						>
							...
							<h3 className="h3 font-bold text-lg">{item.title}</h3>
							<p className="font-light text-base text-surface-600-400 leading-relaxed">
								{item.description}
							</p>
						</div>
			...
			>
				<div className="mb-16 grid grid-cols-1 gap-8 md:grid-cols-2 md:items-end">
					<div data-aos="fade-right">
						...
						<p className="font-light text-lg text-surface-600-400 leading-relaxed">
							Comprehensive docs, examples, and contribution guides — everything
							you need to run, customize, and contribute to Chithi.
						</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
					{data.map((doc, i) => (
						...
						>
							<div className="mb-6">
								<doc.icon size={24} className="text-surface-500-400" />
							</div>
			...
			<section className="border-surface-200-800/50 border-t py-24 md:py-32">
				<div className="mx-auto mb-12 max-w-3xl text-center" data-aos="fade-up">
					...
					<p className="font-light text-lg text-surface-600-400 leading-relaxed">
						Not ready to self-host? Experience Chithi immediately by interacting
						with our community-hosted public instances directly from your
						browser.
					</p>
				</div>
			...
			>
				...
				>
					...
					<p className="mx-auto max-w-2xl font-light text-lg text-surface-600-400 leading-relaxed">
						Join the open-source community protecting their data from big tech,
						analytics tracking, and opaque privacy policies.
					</p>
