<script lang="ts">
  import { SunIcon, MoonIcon, LogOut, UserCog, SlidersVertical, Link, BookOpenText, Gauge, Info } from '@lucide/svelte';
  ...
  type LinkItem = { name: string; href: string; icon: Component<any>; order: number; iconLoader?: () => Promise<{ default: Component<any> }> };;
  ...
  const seen = new Set(baseRightFooterLinks.map((l) => l.href));
  ...
  $effect.pre(() => {
    for (const link of rightFooterLinks) {
      if (!link.iconLoader || iconPromises.has(link.href)) continue;
      const promise = link.iconLoader().then((mod) => { rightFooterLinks = rightFooterLinks.map((i) => i.href === link.href ? { ...i, icon: mod.default } : i); }).catch(() => undefined);
      iconPromises.set(link.href, promise);
    }
  });
</script>

{#snippet footerLink(item: LinkItem)}
  <div style="order:{item.order}">
    <Tooltip.Provider delayDuration={100}>
      <Tooltip.Root>
        <Tooltip.Trigger><Button variant="ghost" size="icon" aria-label={item.name} class="transition-colors hover:text-foreground" href={item.href}><item.icon /></Button></Tooltip.Trigger>
...
<div class="relative flex min-h-svh min-w-screen flex-col overflow-hidden bg-background text-foreground">
  <header class="sticky top-0 z-50 flex items-center justify-between bg-transparent p-4 backdrop-blur-md transition-colors duration-500">
    <a href="/" class="flex items-center gap-2" onclick={samePageNav}><img src={favicon} alt="logo" class="h-6 w-6" /><h1 class="text-2xl font-bold md:text-xl">Chithi</h1></a>
    <div class="flex items-center gap-2">
      {#if userData.data}
        <Dropdown.Root>
          <Dropdown.Trigger><div class="my-0.5"><Avatar.Root>{#if hashedAvatar}<Avatar.Image src={hashedAvatar} alt="@{userData.data.username}" />{/if}<Avatar.Fallback>{initials}</Avatar.Fallback></Avatar.Root></div></Dropdown.Trigger>
          <Dropdown.Content align="end" sideOffset={4} class="w-48">
            <Dropdown.Item onSelect={(e) => e.preventDefault()}>
              <div class="flex w-full items-center justify-between gap-2"><Label class="cursor-pointer" onclick={toggleMode}>Theme</Label><Button onclick={toggleMode} variant="outline" size="icon" class="relative"><SunIcon class={sunClass} /><MoonIcon class={moonClass} /><span class="sr-only">Toggle theme</span></Button></div>
            </Dropdown.Item>
            <Dropdown.Separator />
            <Dropdown.Sub>
              <Dropdown.SubTrigger>Admin</Dropdown.SubTrigger>
              <Dropdown.SubContent>
                {#each adminLinks as item}<Dropdown.Item><a href={item.href} class="flex w-full items-center gap-2" style="order:{item.order}"><item.icon />{item.name}</a></Dropdown.Item>{/each}
              </Dropdown.SubContent>
            </Dropdown.Sub>
            <Dropdown.Item class="mt-1 flex items-center gap-2" variant="destructive"><a href="/logout?next={page.url.pathname}" class="flex w-full items-center gap-2"><LogOut class="h-4 w-4" />Logout</a></Dropdown.Item>
          </Dropdown.Content>
        </Dropdown.Root>
      {:else}
        <Button variant="outline" size="sm" href="/login?next={page.url.pathname}">Login</Button>
        <Button variant="outline" size="icon" onclick={(e) => { e.preventDefault(); toggleMode(); }}><SunIcon class={sunClass} /><MoonIcon class={moonClass} /><span class="sr-only">Toggle theme</span></Button>
      {/if}
    </div>
  </header>
  {#key flagForRestart}<main class="relative flex flex-1 items-center justify-center overflow-hidden p-4"><div class="relative z-10 w-full max-w-5xl shadow-[0_0_15px_-12px_var(--primary)]">{@render children()}</div></main>{/key}
  <footer class="bg-transparent p-4 backdrop-blur-md transition-colors duration-500">
    <div class="mx-auto w-full">
      <nav class="flex flex-row items-center justify-between text-sm text-muted-foreground">
        <div class="flex flex-wrap items-center gap-2 md:gap-6">{#each leftFooterLinks as item}{@render footerLink(item){/each}</div>
        <div class="flex flex-wrap items-center gap-2 md:gap-6">{#each rightFooterLinks as item}{@render footerLink(item){/each}</div>
      </nav>
    </div>
  </footer>
</div>
