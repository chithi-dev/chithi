<script lang="ts">
  ...
  let status = $state<'idle' | 'measuring latency' | 'downloading' | 'uploading' | 'finished' | 'error' | 'starting'>('idle');
  ...
  function startTest() {
    ...
    status = 'starting';
    latency.set(0, { duration: 0 }); downloadSpeed.set(0, { duration: 0 }); uploadSpeed.set(0, { duration: 0 }); progress.set(0, { duration: 0 });
    downloadHistory = []; uploadHistory = []; errorMsg = ''; maxSpeed = 100;
    ...
    worker.onmessage = (e) => {
      ...
      if (type === 'phase') {
        ...
        if (e.data.phase === 'latency') latency.target = speed;
        if (e.data.phase === 'download') { downloadSpeed.target = speed; downloadHistory.push({ progress: e.data.progress, speed }); if (speed > maxSpeed * 0.9) maxSpeed = Math.max(maxSpeed * 2, Math.ceil(speed / 100) * 100 * 1.5); }
        if (e.data.phase === 'upload') { uploadSpeed.target = speed; uploadHistory.push({ progress: e.data.progress, speed }); if (speed > maxSpeed * 0.9) maxSpeed = Math.max(maxSpeed * 2, Math.ceil(speed / 100) * 100 * 1.5); }
      } else if (type === 'result') {
      ...
      } else if (type === 'finish') { status = 'finished'; progress.target = 100; }
      else if (type === 'error') { status = 'error'; errorMsg = e.data.error; }
    };
    worker.postMessage({ type: 'start', duration: testDuration, urls: Api.SPEEDTEST });
  }

  $effect(() => { return () => { if (worker) worker.terminate(); }; });

  const activePhase = $derived(status.includes('download') ? 'download' : status.includes('upload') ? 'upload' : null);
  const currentSpeed = $derived(activePhase === 'download' ? downloadSpeed.current : activePhase === 'upload' ? uploadSpeed.current : 0);
  const canStart = $derived(['idle', 'finished', 'error'].includes(status));
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex h-6 flex-col justify-center">
          <div class={['flex items-center gap-2 transition-opacity duration-200', status === 'idle' || status === 'error' ? 'opacity-0' : 'opacity-100']}>
            {#if status === 'finished'}<span class="text-sm font-medium tracking-wider text-green-500 uppercase">Test Complete</span>
            {:else}<span class="animate-pulse text-sm font-medium tracking-wider text-muted-foreground uppercase">{status}...</span>{/if}
          </div>
        </div>
      </div>
    </CardHeader>
      ...
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div class="flex flex-col items-center justify-center gap-4 rounded-xl border bg-muted/30 p-6 transition-colors">
          ...
          <div class="flex h-36 w-full items-center justify-center md:h-48 xl:h-56">
            <div class="flex items-baseline gap-1"><span class="text-6xl font-bold text-foreground tabular-nums">{latency.current.toFixed(0)}</span><span class="text-xl font-medium text-muted-foreground">ms</span></div>
          </div>
        </div>
        <div class="flex flex-col items-center justify-center gap-4 rounded-xl border bg-muted/30 p-6 transition-colors">
          <div class="flex items-center gap-2 font-semibold text-foreground"><ArrowDown class="h-4 w-4 text-chart-2" />Download Speed</div>
          <div class="relative w-48 xl:w-56"><SpeedGauge value={downloadSpeed.current} activeColor={chartConfig.download.color} unit="Mbps" /></div>
        </div>
        <div class="flex flex-col items-center justify-center gap-4 rounded-xl border bg-muted/30 p-6 transition-colors">
          <div class="flex items-center gap-2 font-semibold text-foreground"><ArrowUp class="h-4 w-4 text-chart-3" />Upload Speed</div>
          <div class="relative w-48 xl:w-56"><SpeedGauge value={uploadSpeed.current} activeColor={chartConfig.upload.color} unit="Mbps" /></div>
        </div>
      </div>
      <div class="w-full pt-4">
        <SpeedGraph {downloadHistory} {uploadHistory} {maxSpeed} {testDuration} downloadColor={chartConfig.download.color} uploadColor={chartConfig.upload.color} {activePhase} {currentSpeed} currentProgress={progress.current / 100} />
      </div>
      <div class="relative h-2 w-full">
        <div class={['absolute inset-0 flex items-center justify-center rounded-md border border-destructive/50 bg-destructive/10 text-center text-sm text-destructive transition-all duration-300', status === 'error' ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-2 opacity-0']}>Error: {errorMsg}</div>
      </div>
      <Separator />
