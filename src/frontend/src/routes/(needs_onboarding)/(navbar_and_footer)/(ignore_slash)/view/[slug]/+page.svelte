<script lang="ts">
  ...
  import { FileText, CircleAlert, LoaderCircle, Download, KeyRound, Eye, File, Folder, ExternalLink, Image as ImageIcon, FileCode, FolderOpen, FilePlay, FileHeadphone } from '@lucide/svelte';
  ...
  const fileIcon = (n: string) => { const l = n.toLowerCase(); if (l.match(/\.(png|jpg|jpeg|gif|webp|svg|ico|bmp|avif|jxl|jxr|hdp|wdp)$/)) return ImageIcon; if (l.match(/\.(mp4|webm|ogv|mov|mkv)$/)) return FilePlay; if (l.match(/\.(mp3|wav|ogg|m4a|aac|flac)$/)) return FileHeadphone; if (l.match(/\.(txt|md|json|js|ts|svelte|html|css|scss|xml|log|csv|sh|yaml|yml|sql|py|java|c|cpp|h|go|rs|php|rb)$/)) return FileCode; return File; };

  const setFileParam = (name: string | null) => { const url = new URL(page.url); name ? url.searchParams.set('file', name) : url.searchParams.delete('file'); goto(url.toString(), { replaceState: true, keepFocus: true, noScroll: true }); };
  ...
  async function fetchAndUnzip() {
    ...
    try {
      ...
      if (fileParam) { const m = zipEntries.find((e) => e.filename === fileParam); if (m) openEntry(m); }
    } catch (e: any) {
      console.error(e);
      if (e instanceof PasswordRequiredError) { phase = 'needs_password'; toast.info('Password required for decryption'); }
        ...
        phase = 'error';
        toast.error(e.message?.includes('missing end marker') ? 'The decrypted archive appears truncated or corrupted.' : 'The decrypted data is not a valid archive.');
      }
      else if (prev === 'needs_password' && password) { toast.error('Incorrect password?'); phase = 'needs_password'; }
      else if (e.name === 'AbortError') phase = 'ready';
      else { toast.error('Failed: ' + e.message); phase = 'error'; }
    }
  }
  ...
  async function openEntry(entry: Entry) {
    ...
    if (text !== null) { viewingFile = { text, url: null, filename: entry.filename }; setFileParam(entry.filename); return; }
    const url = URL.createObjectURL(viewBlob);
    viewingFile = { text: null, url, filename: entry.filename };
    setFileParam(entry.filename);
  }

  function closeViewer() { if (viewingFile?.url) URL.revokeObjectURL(viewingFile.url); viewingFile = null; setFileParam(null); }
  function copyViewerLink() { if (!viewingFile) return; const url = new URL(page.url); url.searchParams.set('file', viewingFile.filename); navigator.clipboard.writeText(url.toString()); }
  async function saveEntry(entry: Entry) { if (entry.directory || !entry.getData) return; const blob = await entry.getData(new BlobWriter(), { password }); const url = URL.createObjectURL(blob); autoDownload(url, entry.filename.split('/').pop() || 'file'); URL.revokeObjectURL(url); }
  function downloadOriginal() { if (!decryptedBlob) return; const name = filename.toLowerCase().endsWith('.zip') ? filename : `${filename}.zip`; const url = URL.createObjectURL(decryptedBlob); autoDownload(url, name); URL.revokeObjectURL(url); }
</script>
...
<Card.Root class="relative z-10 mx-auto w-full max-w-5xl border-border bg-card">
  <Card.Content class="p-6">
    <div class="flex min-h-150 flex-col items-center justify-center">
      {#if status === 'checking'}
        <div class="flex flex-col items-center justify-center py-8"><LoaderCircle class="mb-4 h-8 w-8 animate-spin text-primary" /><p class="text-muted-foreground">Verifying key and file...</p></div>
      {/if}
      {#if status === 'error'}
        {#if !key}
          <div in:fly={{ y: 20, duration: 800 }} class="mx-auto w-full max-w-lg">
            ...
            <Card.Footer class="mt-6 flex w-full flex-col gap-6 px-0"><Button class="w-full" variant="outline" href="/">Go Home</Button></Card.Footer>
          </div>
        {:else}
            ...
            {#if status === 'needs_password'}
              <div class="mx-auto flex w-full max-w-sm flex-col items-center gap-2 py-8">
                <div class="flex w-full items-center">
                  <Input type="password" placeholder="Password" class="rounded-r-none focus-visible:z-10" bind:value={password} onkeydown={(e) => e.key === 'Enter' && fetchAndUnzip()} />
                  <Button class="rounded-l-none" onclick={fetchAndUnzip}>Unlock</Button>
                </div>
            ...
            {:else}
              <div class="mb-6 flex items-center gap-4 rounded-lg border bg-background/50 p-4">
                <div class="rounded bg-primary/10 p-2 text-primary">
                  {#if status === 'downloading' || status === 'unzipping'}<LoaderCircle class="h-6 w-6 animate-spin" />{:else}<FileText class="h-6 w-6" />{/if}
                </div>
                <div class="flex-1 overflow-hidden"><p class="truncate font-medium">{filename}</p><p class="text-xs text-muted-foreground">{formatFileSize(fileSize)}</p></div>
              </div>
                  ...
                  <div class="w-full space-y-2">
                    ...
                    <div class="flex justify-between text-xs text-muted-foreground">
                      ...
                      <span class="flex items-center">
                        {#if status === 'unzipping'}<FolderOpen class="mr-2 h-3 w-3 animate-pulse" />Unzipping...{:else}<Download class="mr-2 h-3 w-3 animate-bounce" />Decrypting...{/if}
                      </span>
                    </div>
                  </div>
                {:else}
      ...
      {#if status === 'listing'}
        {#if viewingFile}
          <div class="w-full" in:fade={{ duration: 200 }}>
            <FileViewerOverlay filename={viewingFile.filename} contentText={viewingFile.text} contentUrl={viewingFile.url}
              onclose={closeViewer} oncopylink={copyViewerLink}
              ondownload={() => { if (!viewingFile) return; const blobUrl = viewingFile.url || URL.createObjectURL(new Blob([viewingFile.text!], { type: 'text/plain' })); autoDownload(blobUrl, viewingFile.filename.split('/').pop() || 'file'); if (!viewingFile.url) URL.revokeObjectURL(blobUrl); }} />
          </div>
        {:else}
          <div class="w-full" in:fade={{ duration: 300 }}>
            <div class="mb-4 flex items-center justify-between">
              <div><h2 class="text-xl font-bold">Contents of {filename}</h2><p class="text-sm text-muted-foreground">{zipEntries.length} items found</p></div>
                  ...
                  {#each zipEntries as entry}
                    <div class="group flex w-full items-center gap-3 rounded-md p-2 transition-colors hover:bg-muted/50">
                      <button class="flex flex-1 cursor-pointer items-center gap-3 overflow-hidden border-0 bg-transparent p-0 text-left" onclick={() => openEntry(entry)}>
                        {#if entry.directory}<Folder class="h-5 w-5 shrink-0 text-primary" />{:else}{@const Icon = fileIcon(entry.filename)}<Icon class="h-5 w-5 shrink-0 text-primary" />{/if}
                        <div class="flex-1 overflow-hidden"><p class="truncate text-sm font-medium">{entry.filename}</p>{#if !entry.directory}<p class="text-xs text-muted-foreground">{formatFileSize(entry.uncompressedSize)}</p>{/if}</div>
                      </button>
                      {#if !entry.directory}
