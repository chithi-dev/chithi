<script lang="ts">
  ...
  let uploadResult = $state<{ finalLink: string; viewOnceLink: string; isViewOnce: boolean } | null>(null);
  let hasMounted = $state(false);

  const detailsMarkdown = $derived(configData.data?.site_description ?? '');
  let detailsHtml = $state('');

  $effect(() => { if (detailsMarkdown) markdown_to_html(detailsMarkdown).then((h) => { detailsHtml = h; }); });

  // Guard: skip if Stage 3
  const g3 = (e?: Event) => { if (stage !== UploadStage.Stage_3) { e?.preventDefault?.(); return true; } return false; };

  const handleMouseBack = (e: MouseEvent) => { if (e.button === 3 && stage === UploadStage.Stage_2) { stage = UploadStage.Stage_1; e.preventDefault(); } };
  ...
  const resetState = (mode: 'push' | 'replace' = 'push') => {
    ...
    if (mode === 'replace') window.history.replaceState({ stage: UploadStage.Stage_1 }, '', window.location.href);
    else window.history.pushState({ stage: UploadStage.Stage_1 }, '', window.location.href);
  };

  const onBack = () => { stage = UploadStage.Stage_1; };

  onMount(() => { queueMicrotask(() => { resetState('replace'); hasMounted = true; }); });
</script>

<svelte:window ondragenter={handleWindowDragEnter} ondragover={handleWindowDragOver} ondragleave={handleWindowDragLeave} ondrop={handleWindowDrop} onpaste={handlePaste} onauxclick={handleMouseBack} onpointerdown={handleMouseBack} onpopstate={(e) => { if (hasMounted) stage = isWhichUploadStage(e.state?.stage) ? e.state.stage : UploadStage.Stage_1; }} />
...
{#snippet configSkeleton()}
  <div class="relative flex h-full w-full flex-col items-center justify-center rounded-lg bg-card p-12">
    <svg class="pointer-events-none absolute inset-0 h-full w-full rounded-lg">
      <rect width="100%" height="100%" rx="8" ry="8" fill="none" stroke="currentColor" stroke-width="2" class="text-border" stroke-dasharray="10" />
    </svg>
...
{#snippet rightColumnSkeleton()}
  <div class="flex h-full w-full flex-col p-4 lg:p-8">
    <ScrollArea class="h-auto w-full lg:h-full">
      <div class="prose w-full max-w-none prose-zinc md:text-sm lg:text-lg lg:leading-relaxed dark:prose-invert">
        <Skeleton class="mb-4 h-8 w-1/2" />
        <div class="space-y-2">
          <Skeleton class="h-4 w-full" /><Skeleton class="h-4 w-full" /><Skeleton class="h-4 w-2/4" /><br />
...
<Card.Root class={['relative z-10 mx-auto w-full max-w-5xl border-border bg-card transition-all duration-200', dragActive && 'shadow-[0_0_20px_-10px_var(--primary)]', dragOverCard && 'shadow-[0_0_40px_-10px_var(--primary)]', dragOverZone && 'shadow-[0_0_60px_-10px_var(--primary)]']}
  ondrop={(e) => { if (!g3(e)) return; resetDrag(); toast.error('File/Folder must be dropped in the bordered area'); }}
  ...
  <Card.Content class="p-6">
    <div class="grid min-h-150 grid-cols-1 gap-8 lg:grid-cols-2">
      ...
      {:else if configData.data?.allow_uploads === false}
        <div class="col-span-1 flex min-h-100 flex-col items-center justify-center p-4 text-center lg:col-span-2 lg:p-8">
          <div class="mb-4 rounded-full bg-muted/50 p-4 lg:mb-6 lg:p-6"><CloudOff class="h-10 w-10 text-muted-foreground lg:h-12 lg:w-12" /></div>
          <h2 class="mb-2 text-2xl font-bold md:text-xl lg:mb-4 lg:text-3xl">Uploads Disabled</h2>
          <p class="max-w-lg text-muted-foreground md:mb-4 md:text-sm lg:text-lg lg:leading-relaxed">The administrator of this instance has temporarily disabled new file uploads. Existing files can still be downloaded.</p>
        </div>
      {:else if stage === UploadStage.Stage_1}
        <div in:fly={{ x: -20, duration: 400 }}>
          <Stage1 {onFilesSelected} isDraggingOverZone={dragOverZone} onZoneDragEnter={handleZoneDragEnter} onZoneDragLeave={handleZoneDragLeave} />
        </div>
        <div class="flex h-full w-full flex-col p-4 lg:p-8" in:fade>
          <ScrollArea class="h-auto w-full lg:h-full">
            <div class="prose w-full max-w-none prose-zinc md:text-sm lg:text-lg lg:leading-relaxed dark:prose-invert">{@html detailsHtml}</div>
          </ScrollArea>
        </div>
      {:else if stage === UploadStage.Stage_2}
        <div in:fly={{ x: 20, duration: 400 }}>
          <Stage2 bind:files onFilesUpdated={(f) => (files = f)} {onUploadComplete} {onBack} isDraggingOverZone={dragOverZone} onZoneDragEnter={handleZoneDragEnter} onZoneDragLeave={handleZoneDragLeave} />
        </div>
      ...
      {:else if stage === UploadStage.Stage_3 && uploadResult}
        <div class="col-span-1 lg:col-span-2" in:fly={{ y: 20, duration: 400 }}>
          <Stage3 finalLink={uploadResult.finalLink} viewOnceLink={uploadResult.viewOnceLink} isViewOnce={uploadResult.isViewOnce} onReset={() => resetState('push')} />
        </div>
      {/if}
    </div>
  </Card.Content>
...
<UploadShowcase localUploadSize={stage === UploadStage.Stage_2 ? files.reduce((s, f) => s + f.size, 0) : 0} />

{#if dev}
  <div class="fixed bottom-4 left-4 z-50"><Button onclick={() => (debugLoading = !debugLoading)}>Toggle Skeleton</Button></div>
{/if}
