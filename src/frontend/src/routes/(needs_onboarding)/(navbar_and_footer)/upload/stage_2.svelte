<script lang="ts">
  ...
  const addFiles = (newFiles: File[]) => {
    ...
    if (configData.data?.max_file_size_limit && rawTotalSize + newSize > configData.data.max_file_size_limit) {
...
{:else if files.length}
  <div class="flex h-full w-full flex-col pb-2">
    ...
    <ScrollArea class={['mb-4 h-60 max-h-[45vh] w-full rounded-lg border border-border bg-card transition-colors lg:max-h-[50vh] lg:flex-1', isDraggingOverZone && 'bg-primary/5']} ondragenter={onZoneDragEnter} ondragleave={onZoneDragLeave} ondragover={(e) => e.preventDefault()} ondrop={handleZoneDrop}>
      <div class="p-4">
            ...
            <Select.Content>
              {#if configData.data?.download_configs}
                {#each configData.data.download_configs as limit}
                  <Select.Item value={limit.toString()}>{limit} {fmtUnit(limit, 'downloads')}</Select.Item>
                {/each}
              {:else}
      ...
      <div class="flex h-9 items-center gap-2">
        ...
        {#if isPasswordProtected}
          <div class="relative max-w-xs flex-1">
            ...
            <button type="button" onclick={() => (showPassword = !showPassword)} class="absolute top-1 right-3 text-muted-foreground hover:text-foreground">
