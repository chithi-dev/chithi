<script lang="ts">
	...
	$effect(() => {
		...
		else if (!files.length) folderName = uuidv7();
	});
	$effect(() => {
	...
	const addFiles = (newFiles: File[]) => {
		if (
			...
			rawTotalSize + newFiles.reduce((s, f) => s + f.size, 0) > configData.data.max_file_size_limit
		) {
			toast.error(
	...
	const handleZoneDrop = async (e: DragEvent) => {
		...
		if (e.dataTransfer?.items) {
			const nf = await clipboardFiles(e.dataTransfer.items);
	...
	const handleUpload = async (viewOnce = false) => {
		...
		try {
			...
			const { stream: encryptedStream, keySecret } = await createEncryptedStream(
				...
				rawTotalSize,
				(p, t) => {
					if (t && t > 0) encryptionProgress.target = Math.min(100, Math.round((p / t) * 100));
