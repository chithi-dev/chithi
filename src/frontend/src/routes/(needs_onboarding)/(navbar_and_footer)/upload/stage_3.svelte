<script lang="ts">
	...
	const activeLink = $derived(isViewOnce ? viewOnceLink : finalLink);
...
<div class="col-span-1 flex h-full flex-col items-center justify-center py-12 text-center lg:col-span-2">
	...
	<div class="mb-8 flex w-full max-w-md items-center gap-2">
		<Input readonly value={activeLink} class="font-mono text-sm" />
	</div>

	<div class="mb-8 flex flex-col items-center gap-4">
		<div class="rounded-lg border bg-white p-2 dark:bg-white">
			<QRCode value={activeLink} size={180} color="#000000" backgroundColor="#ffffff" />
		</div>
	</div>
		...
		<ButtonGroup.Root>
			<Tooltip.Provider>
				...
				{:else}
					<Tooltip.Root>
						<Tooltip.Trigger>
							<Button variant="outline" size="icon-sm" href={finalLink} class="cursor-pointer" aria-label="Download">
