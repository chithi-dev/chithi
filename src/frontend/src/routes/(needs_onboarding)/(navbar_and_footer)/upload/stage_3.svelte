...
<div class="col-span-1 flex h-full flex-col items-center justify-center py-12 text-center lg:col-span-2">
  ...
  <div class="mb-8 flex flex-col items-center gap-4">
    <div class="rounded-lg border bg-white p-2 dark:bg-white"><QRCode value={link} size={180} color="#000000" backgroundColor="#ffffff" /></div>
  </div>
  ...
  <div class="flex flex-col gap-4">
    <ButtonGroup.Root>
      <Tooltip.Provider>
        <Tooltip.Root>
          <Tooltip.Trigger><Button variant="outline" size="sm" onclick={copyLink} class="w-32">
        ...
        {#if isViewOnce}
          <Tooltip.Root>
            <Tooltip.Trigger><Button variant="outline" size="icon-sm" href={viewOnceLink} aria-label="View once"><ScanEye class="size-4" /></Button></Tooltip.Trigger>
            <Tooltip.Content>View once in browser</Tooltip.Content>
          </Tooltip.Root>
        {:else}
          <Tooltip.Root>
            <Tooltip.Trigger><Button variant="outline" size="icon-sm" href={finalLink} aria-label="Download"><Download class="size-4" /></Button></Tooltip.Trigger>
            <Tooltip.Content>Download file</Tooltip.Content>
          </Tooltip.Root>
          <Tooltip.Root>
            <Tooltip.Trigger><Button variant="outline" size="icon-sm" href={finalLink.replace('/download/', '/view/')} aria-label="View"><Eye class="size-4" /></Button></Tooltip.Trigger>
