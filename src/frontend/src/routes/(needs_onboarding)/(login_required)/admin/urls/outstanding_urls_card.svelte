...
<Card.Root class="border bg-background">
	...
	<div class="px-6 pb-4">
		<div class="relative max-w-sm">
			...
			<Input placeholder="Filter by name..." bind:value={globalFilter} class="pl-9" />
		</div>
	</div>
				...
				{#each table.getHeaderGroups() as headerGroup}
					<Table.Row>
						{#each headerGroup.headers as header}
							<Table.Head
								class={header.column.id === 'size' ||
								header.column.id === 'created_at' ||
								header.column.id === 'downloads'
									? 'cursor-pointer select-none'
									: ''}
								onclick={header.column.id === 'size'
									? () => toggleSort('size')
									: header.column.id === 'created_at'
										? () => toggleSort('created_at')
										: header.column.id === 'downloads'
											? () => toggleSort('downloads')
											: undefined}
							>
								<FlexRender
