...
<form use:enhance method="POST" class="grid gap-6">
  <Form.Field {form} name="email">
    <Form.Control>
      ...
      <div class="group relative">
        ...
        <Input name="email" bind:value={$formData.email} placeholder="name@example.com" class="h-12 border-border bg-background/50 pl-11 transition-all focus-visible:ring-primary/40" />
      </div>
    </Form.Control>
    <Form.FieldErrors />
  </Form.Field>
  <Form.Field {form} name="password">
    <Form.Control>
      ...
      <div class="group relative">
        ...
        <Input name="password" type={showPassword ? 'text' : 'password'} bind:value={$formData.password} placeholder="••••••••" class="h-12 border-border bg-background/50 px-11 transition-all focus-visible:ring-primary/40" />
  ...
  <Form.Button disabled={$submitting} class="h-12 w-full font-semibold shadow-lg shadow-primary/20 transition-all hover:brightness-105 active:scale-[0.98] disabled:opacity-70">
    {#if $submitting}<LoaderCircle class="mr-2 size-5 animate-spin" />Authenticating{:else}Sign In<ArrowRight class="ml-2 size-5 transition-transform group-hover:translate-x-1" />{/if}
  </Form.Button>
</form>
