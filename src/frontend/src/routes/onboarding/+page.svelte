...
<div class="relative flex min-h-svh items-center justify-center overflow-hidden bg-slate-50 p-4 transition-colors duration-500 dark:bg-zinc-950">
  <FancyGrid />
      ...
      <div in:fade class="mx-auto w-full max-w-md">
        <Card.Root class="relative overflow-hidden border-slate-200/60 bg-white/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-2xl dark:border-zinc-800/50 dark:bg-zinc-900/50">
          ...
          <Card.Header class="space-y-3 pt-10 pb-6 text-center">
            ...
            <div class="space-y-1"><Card.Title class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Already Onboarded</Card.Title><Card.Description class="text-sm text-slate-500 dark:text-zinc-400">This instance has already been set up.</Card.Description></div>
          </Card.Header>
          <Card.Content><div class="grid gap-4"><p class="text-center text-sm text-slate-600 dark:text-zinc-400">If you need to reconfigure, sign in and adjust settings from the dashboard.</p><Button class="mt-2 h-12 w-full" onclick={() => goto('/')}>Go to dashboard</Button></div></Card.Content>
        </Card.Root>
      </div>
