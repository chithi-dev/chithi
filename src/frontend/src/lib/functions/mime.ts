export function detectMimeFromBytes(bytes: Uint8Array): string | null {
  ...
  if (has(0, 0xff, 0x0a) || has(0, 0x00, 0x00, 0x00, 0x0c, 0x4a, 0x58, 0x4c, 0x20, 0x0d, 0x0a, 0x87, 0x0a))
    return 'image/jxl';
  ...
  if (/<!doctype\s*html/i.test(textSample) || /^\s*<[hH][tT][mM][lL]/.test(textSample)) return 'text/html';
  if (/^\s*<\?xml/i.test(textSample)) return 'application/xml';
