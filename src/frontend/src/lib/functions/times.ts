const T_VALS = { Seconds: 1, Minutes: 60, Hours: 3600, Days: 86400, Weeks: 604800, Months: 2592000 } as const;
...
export function formatSeconds(seconds: number): { val: number; unit: TimeUnit } {
  ...
  for (const unit of [...T_UNITS].reverse()) {
    if (seconds >= T_VALS[unit]) return { val: parseFloat((seconds / T_VALS[unit]).toFixed(2)), unit };
  }
